...
public class Image
{
    ...
    public void getData(String filePath, String outFolderPath) throws Exception
    {
    	...
    	for(String sfileName : alFileList)
    	{
            ...
            FileWriter fstream = new FileWriter(outFolderPath+"out"+sequence+"_"+image.getHeight()+"x"+image.getWidth()+".txt");
    ...
    public void setData(String filename, String imageFile) throws Exception
    {
       ...
       //height =512;
        ...
        for(int i=0;i<height/3;i++)
        {
        	...
        	{
        		newImage.setRGB(j, i,((aValue[i][j]<<16)+(aValue[(height/3)+i][j]<<8) + aValue[(2*(height/3))+i][j]));
        	}
        }
    ...
    public String getMessage(int[] bitArray)
    {
    	...
    	{
    		sMessage = sMessage+(char)Integer.parseInt((bitArray[(7*i)]+""+bitArray[((7*i)+1)]+""+bitArray[((7*i)+2)]+""+bitArray[((7*i)+3)]+""+bitArray[((7*i)+4)]+""+bitArray[((7*i)+5)]+""+bitArray[((7*i)+6)]),2);
